...
const OnboardingWizard = ({
  ...
  const validateCompanyCode = async (code: string) => {
    ...
    const {
      data: validCode
    } = await supabase.from('company_codes').select('code').eq('code', numericCode).maybeSingle();
    if (!validCode) {
    ...
    try {
      ...
      if (companyCode) {
        const {
          data: success,
          error: functionError
        } = await supabase.rpc('set_user_company', {
      ...
      <DialogContent hideCloseButton={true} className="sm:max-w-[800px]">
        <div className="grid gap-6 py-4">
          <div className="flex justify-between items-center">
            <h2 className="text-2xl font-semibold">Company code</h2>
            <div className="text-sm text-muted-foreground">
              Step {step} of 3
            </div>
          </div>

          {step === 1 ? <WelcomeStep onNext={handleNext} /> : step === 2 ? <CompanyCodeStep companyCode={companyCode} onCompanyCodeChange={code => {
          setCompanyCode(code);
          setError("");
        }} onNext={handleNext} onPrevious={handlePrevious} /> : <UserDescriptionStep userDescription={userDescription} onDescriptionChange={setUserDescription} onPrevious={handlePrevious} onSave={handleSave} error={error} isSubmitting={isSubmitting} />}
        </div>
      </DialogContent>
    </Dialog>;
};
